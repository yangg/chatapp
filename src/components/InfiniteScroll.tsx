  ...
  return (
      <>
        ...
        <div className={`flex justify-center items-center pb-4 empty:hidden ${!initialized ? 'flex-1' : ''}`}>
          {initialized && !loading && !error && !hasMore && <><Separator className={'flex-1'}/><span className={'text-xs px-3'}>No more data</span><Separator className={'flex-1'}/></>}
          {loading && <><Loader2 className="h-6 w-6 animate-spin"/> <span
            className={'text-xs ml-2'}>Loading...</span></>}
          {error && <><CircleAlert className="ml-1 size-4 text-destructive"/><span
            className={'text-xs ml-2 text-destructive'}>{error.message}</span></>}
        </div>
      </>
  )
}
